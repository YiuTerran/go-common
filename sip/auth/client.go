...
import (
	"fmt"
	deepcopy "github.com/YiuTerran/go-common/base/util/copyutil"
...
func (auth *Authorization) Value() string {
	return auth.String()
}

func (auth *Authorization) Clone() sip.Header {
	return deepcopy.DeepCopy(auth)
}

func (auth *Authorization) Equals(other any) bool {
	if other == nil {
		return false
	}
	var rhs *Authorization
	if p, ok := other.(*Authorization); !ok {
		if t, ok := other.(Authorization); !ok {
			return false
		} else {
			rhs = &t
		}
	...
	return rhs.String() == auth.String()
}

var (
	authPattern = regexp.MustCompile(`([\w]+)=("([^"]+)"|([\w]+))`)
)

func OfString(value string) *Authorization {
	auth := &Authorization{
		algorithm: "MD5",
		cnonce:    geneGrateNonce(12),
		opaque:    "",
		stale:     "",
		qop:       "",
		nc:        0,
...
func AuthorizeRequest(request sip.Request, response sip.Response, user, password sip.MaybeString) error {
	...
	if headers := response.GetHeaders(authenticateHeaderName); len(headers) > 0 {
		authenticateHeader := headers[0].(*sip.GenericHeader)
		auth := OfString(authenticateHeader.Contents).
		...
		headers = request.GetHeaders(authorizeHeaderName)
		if len(headers) > 0 {
			authorizationHeader = headers[0].(*sip.GenericHeader)
