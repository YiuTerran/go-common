...
var _ = Describe("ConnectionHandler", func() {
	...
	Context("just initialized", func() {
		...
		BeforeEach(func() {
			...
			conn = transport.NewConnection(server, key, "tcp", log.Fields{})
		})
		AfterEach(func() {
			_ = client.Close()
			_ = server.Close()
		...
		JustBeforeEach(func() {
			handler = transport.NewConnectionHandler(conn, ttl, output, errs, nil, log.Fields{})
		})
	...
	Context("serving connection", func() {
		...
		BeforeEach(func() {
			...
			client = &testutils.MockConn{Conn: c1, LAddr: c1.LocalAddr(), RAddr: addr}
			server = &testutils.MockConn{Conn: c2, LAddr: addr, RAddr: c2.RemoteAddr()}
			conn = transport.NewConnection(server, "dummy", "tcp", log.Fields{})
			wg = &sync.WaitGroup{}
		})
		AfterEach(func() {
			defer func() { recover() }()
...
var _ = Describe("ConnectionPool", func() {
	...
	Context("just initialized", func() {
		BeforeEach(func() {
			...
			pool = transport.NewConnectionPool(output, errs, cancel, nil, log.Fields{})
		})
	...
	Context("that canceled", func() {
		...
		It("should decline Put", func() {
			err = pool.Put(transport.NewConnection(server, key1, "tcp", log.Fields{}), 0)
	...
	Context("that working", func() {
		...
		createConn := func(addr net.Addr) (transport.Connection, transport.Connection) {
			c1, c2 := net.Pipe()
			client := transport.NewConnection(&testutils.MockConn{Conn: c1, LAddr: c1.LocalAddr(), RAddr: addr},
				"", "tcp", log.Fields{})
			server := transport.NewConnection(&testutils.MockConn{Conn: c2, LAddr: addr, RAddr: c2.RemoteAddr()},
				transport.ConnectionKey(addr.String()), "tcp", log.Fields{})
		...
		Context("put connection with empty key = ''", func() {
			BeforeEach(func() {
				_, c2 := net.Pipe()
				server := transport.NewConnection(&testutils.MockConn{Conn: c2, LAddr: addr1, RAddr: c2.RemoteAddr()},
					"", "tcp", log.Fields{})
		...
		Context("has multiple connections: key1=>server1 (TTL=0), key2=>server2 (TTL=time.Millisecond), "+
			...
			Context("when new data arrives from clients", func() {
				...
				It("should pipe handler outputs to self outputs", func() {
