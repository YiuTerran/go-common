...
func TestParams(t *testing.T) {
	doTests([]test{
		// TEST: ParseHeaderParams
		{
			&paramInput{";foo=bar", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}), 8},
		},
		{
			&paramInput{";foo=", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{""}), 5},
		},
		{
			&paramInput{";foo", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", nil), 4},
		},
		{
		...
		{
			&paramInput{"foo!h;l!o", ';', ';', '!', false, true},
			&paramResult{fail, sip.NewParams(sip.HeaderParams), 0},
		},
		{
			&paramInput{"foo;h;l!o", ';', ';', '!', false, true},
			&paramResult{fail, sip.NewParams(sip.HeaderParams), 0},
		},
		{
			...
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}), 17},
		},
		{
		...
		},
		{
			...
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", nil), 12},
		},
		{
			&paramInput{";foo;baz=boop", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", nil).Add("baz", sip.String{"boop"}), 13},
		},
		{
			&paramInput{";foo=bar;baz=boop;a=b", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}).Add("a", sip.String{"b"}), 21},
		},
		{
			&paramInput{";foo;baz=boop;a=b", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", nil).Add("baz", sip.String{"boop"}).Add("a", sip.String{"b"}), 17},
		},
		{
			&paramInput{";foo=bar;baz;a=b", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", nil).Add("a", sip.String{"b"}), 16},
		},
		{
			&paramInput{";foo=bar;baz=boop;a", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}).Add("a", nil), 19},
		},
		{
			&paramInput{";foo=bar;baz=;a", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{""}).Add("a", nil), 15},
		},
		{
			&paramInput{";foo=;baz=bob;a", ';', ';', 0, false, true},
			&paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{""}).Add("baz", sip.String{"bob"}).Add("a", nil), 15},
		},
		{
			&paramInput{"foo=bar", ';', ';', 0, false, true},
			&paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		...
		{&paramInput{"$foo#hello", '$', ',', '#', false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", nil), 4}},
		{&paramInput{"$foo=bar!h;,!o", '$', ',', '!', false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}), 8}},
		{&paramInput{"$foo!h;l!,", '$', ',', '!', false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", nil), 4}},
		{&paramInput{"foo!h;l!o", '$', ',', '!', false, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{"foo,h,l!o", '$', ',', '!', false, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{"$foo=bar,baz=boop", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}), 17}},
		{&paramInput{"$foo=bar;baz", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar;baz"}), 12}},
		{&paramInput{"$foo=bar,baz=boop!lol", '$', ',', '!', false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}), 17}},
		{&paramInput{"$foo=bar,baz", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", nil), 12}},
		{&paramInput{"$foo=,baz", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{""}).Add("baz", nil), 9}},
		{&paramInput{"$foo,baz=boop", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", nil).Add("baz", sip.String{"boop"}), 13}},
		{&paramInput{"$foo=bar,baz=boop,a=b", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}).Add("a", sip.String{"b"}), 21}},
		{&paramInput{"$foo,baz=boop,a=b", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", nil).Add("baz", sip.String{"boop"}).Add("a", sip.String{"b"}), 17}},
		{&paramInput{"$foo=bar,baz,a=b", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", nil).Add("a", sip.String{"b"}), 16}},
		{&paramInput{"$foo=bar,baz=boop,a", '$', ',', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}).Add("a", nil), 19}},
		{&paramInput{";foo", ';', ';', 0, false, false}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo=", ';', ';', 0, false, false}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{""}), 5}},
		{&paramInput{";foo=bar;baz=boop", ';', ';', 0, false, false}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{"boop"}), 17}},
		{&paramInput{";foo=bar;baz", ';', ';', 0, false, false}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo;bar=baz", ';', ';', 0, false, false}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo=;baz=boop", ';', ';', 0, false, false}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{""}).Add("baz", sip.String{"boop"}), 14}},
		{&paramInput{";foo=bar;baz=", ';', ';', 0, false, false}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{""}), 13}},
		{&paramInput{"$foo=bar,baz=,a=b", '$', ',', 0, false, true}, &paramResult{pass,
			sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}).Add("baz", sip.String{""}).Add("a", sip.String{"b"}), 17}},
		{&paramInput{"$foo=bar,baz,a=b", '$', ',', 0, false, false}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 17}},
		{&paramInput{";foo=\"bar\"", ';', ';', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"\"bar\""}), 10}},
		{&paramInput{";foo=\"bar", ';', ';', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"\"bar"}), 9}},
		{&paramInput{";foo=bar\"", ';', ';', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar\""}), 9}},
		{&paramInput{";\"foo\"=bar", ';', ';', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("\"foo\"", sip.String{"bar"}), 10}},
		{&paramInput{";foo\"=bar", ';', ';', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo\"", sip.String{"bar"}), 9}},
		{&paramInput{";\"foo=bar", ';', ';', 0, false, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("\"foo", sip.String{"bar"}), 9}},
		{&paramInput{";foo=\"bar\"", ';', ';', 0, true, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}), 10}},
		{&paramInput{";foo=\"ba\"r", ';', ';', 0, true, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo=ba\"r", ';', ';', 0, true, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo=bar\"", ';', ';', 0, true, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo=\"bar", ';', ';', 0, true, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";\"foo\"=bar", ';', ';', 0, true, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";\"foo=bar", ';', ';', 0, true, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo\"=bar", ';', ';', 0, true, true}, &paramResult{fail, sip.NewParams(sip.HeaderParams), 0}},
		{&paramInput{";foo=\"bar;baz\"", ';', ';', 0, true, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar;baz"}), 14}},
		{&paramInput{";foo=\"bar;baz\";a=b", ';', ';', 0, true, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar;baz"}).Add("a", sip.String{"b"}), 18}},
		{&paramInput{";foo=\"bar;baz\";a", ';', ';', 0, true, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar;baz"}).Add("a", nil), 16}},
		{&paramInput{";foo=bar", ';', ';', 0, true, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"}), 8}},
		{&paramInput{";foo=", ';', ';', 0, true, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{""}), 5}},
		{&paramInput{";foo=\"\"", ';', ';', 0, true, true}, &paramResult{pass, sip.NewParams(sip.HeaderParams).Add("foo", sip.String{""}), 7}},
	}, t)
}

func TestSipUris(t *testing.T) {
	doTests([]test{
		{sipUriInput("sip:bob@example.com"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sip:bob@192.168.0.1"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "192.168.0.1", FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sip:bob:Hunter2@example.com"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: sip.String{"Hunter2"}, FHost: "example.com", FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sips:bob:Hunter2@example.com"), &sipUriResult{pass, sip.SipUri{FIsEncrypted: true, FUser: sip.String{"bob"}, FPassword: sip.String{"Hunter2"},
			FHost: "example.com", FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sips:bob@example.com"), &sipUriResult{pass, sip.SipUri{FIsEncrypted: true, FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sip:example.com"), &sipUriResult{pass, sip.SipUri{FUser: nil, FPassword: nil, FHost: "example.com", FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("example.com"), &sipUriResult{fail, sip.SipUri{}}},
		{sipUriInput("bob@example.com"), &sipUriResult{fail, sip.SipUri{}}},
		{sipUriInput("sip:bob@example.com:5060"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5060, FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sip:bob@88.88.88.88:5060"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "88.88.88.88", FPort: &port5060, FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sip:bob:Hunter2@example.com:5060"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: sip.String{"Hunter2"},
			FHost: "example.com", FPort: &port5060, FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sip:bob@example.com:5"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5, FUriParams: nil, FHeaders: nil}}},
		{sipUriInput("sip:bob@example.com;foo=bar"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com",
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", sip.String{"bar"}), FHeaders: nil}}},
		...
		{sipUriInput("sip:bob@example.com:5;foo"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5,
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", nil), FHeaders: nil}}},
		{sipUriInput("sip:bob@example.com:5;foo;baz=bar"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5,
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", nil).Add("baz", sip.String{"bar"}), FHeaders: nil}}},
		...
		{sipUriInput("sip:bob@example.com:5;foo;baz=bar;a=b"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5,
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", nil).Add("baz", sip.String{"bar"}).Add("a", sip.String{"b"}), FHeaders: nil}}},
		{sipUriInput("sip:bob@example.com:5;baz=bar;foo;a=b"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5,
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", nil).Add("baz", sip.String{"bar"}).Add("a", sip.String{"b"}), FHeaders: nil}}},
		{sipUriInput("sip:bob@example.com?foo=bar"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com",
			FUriParams: nil, FHeaders: sip.NewParams(sip.UriHeaders).Add("foo", sip.String{"bar"})}}},
		{sipUriInput("sip:bob@example.com?foo="), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com",
			FUriParams: nil, FHeaders: sip.NewParams(sip.UriHeaders).Add("foo", sip.String{""})}}},
		{sipUriInput("sip:bob@example.com:5060?foo=bar"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5060,
			FUriParams: nil, FHeaders: sip.NewParams(sip.UriHeaders).Add("foo", sip.String{"bar"})}}},
		...
		{sipUriInput("sips:bob@example.com:5?baz=bar&foo=&a=b"), &sipUriResult{pass, sip.SipUri{FIsEncrypted: true, FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5,
			FUriParams: nil, FHeaders: sip.NewParams(sip.UriHeaders).Add("baz", sip.String{"bar"}).Add("a", sip.String{"b"}).Add("foo", sip.String{""})}}},
		...
		{sipUriInput("sip:bob@example.com;foo?foo=bar"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com",
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", nil),
			FHeaders:   sip.NewParams(sip.UriHeaders).Add("foo", sip.String{"bar"})}}},
		...
		{sipUriInput("sips:bob@example.com:5;foo?baz=bar&a=b&foo="), &sipUriResult{pass, sip.SipUri{FIsEncrypted: true, FUser: sip.String{"bob"},
			...
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", nil),
			FHeaders:   sip.NewParams(sip.UriHeaders).Add("baz", sip.String{"bar"}).Add("a", sip.String{"b"}).Add("foo", sip.String{""})}}},
		...
		{sipUriInput("sip:bob@example.com;foo=baz?foo=bar"), &sipUriResult{pass, sip.SipUri{FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com",
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", sip.String{"baz"}),
			FHeaders:   sip.NewParams(sip.UriHeaders).Add("foo", sip.String{"bar"})}}},
		...
		{sipUriInput("sips:bob@example.com:5;foo=baz?baz=bar&a=b"), &sipUriResult{pass, sip.SipUri{FIsEncrypted: true, FUser: sip.String{"bob"}, FPassword: nil, FHost: "example.com", FPort: &port5,
			FUriParams: sip.NewParams(sip.UriParams).Add("foo", sip.String{"baz"}),
			FHeaders:   sip.NewParams(sip.UriHeaders).Add("baz", sip.String{"bar"}).Add("a", sip.String{"b"})}}},
...
var (
	fooBarHP = sip.NewParams(sip.HeaderParams).Add("foo", sip.String{"bar"})
	fooBarUP = sip.NewParams(sip.UriParams).Add("foo", sip.String{"bar"})
	fooBarUH = sip.NewParams(sip.UriHeaders).Add("foo", sip.String{"bar"})
	fooNilHP = sip.NewParams(sip.HeaderParams).Add("foo", nil)
	fooNilUP = sip.NewParams(sip.UriParams).Add("foo", nil)
	fooNilUH = sip.NewParams(sip.UriHeaders).Add("foo", nil)
)
	...
	doTests([]test{
		{toHeaderInput("To: \"Alice Liddell\" <sip:alice@wonderland.com>"), &toHeaderResult{pass,
			&sip.ToHeader{DisplayName: sip.String{"Alice Liddell"},
				Address: &sip.SipUri{false, sip.String{"alice"}, nil, "wonderland.com", nil, nil, nil},
				Params:  nil}}},
		...
		{toHeaderInput("To: \"Alice Liddell\" <sip:alice@wonderland.com;foo=bar>"), &toHeaderResult{pass,
			&sip.ToHeader{DisplayName: sip.String{"Alice Liddell"},
				Address: &sip.SipUri{false, sip.String{"alice"}, nil, "wonderland.com", nil, fooBarUP, nil},
				Params:  nil}}},

		{toHeaderInput("To: \"Alice Liddell\" <sip:alice@wonderland.com?foo=bar>"), &toHeaderResult{pass,
			&sip.ToHeader{DisplayName: sip.String{"Alice Liddell"},
				Address: &sip.SipUri{false, sip.String{"alice"}, nil, "wonderland.com", nil, nil, fooBarUH},
				Params:  nil}}},
		...
		{toHeaderInput("TO: \"Alice Liddell\" <sip:alice@wonderland.com;foo>"), &toHeaderResult{pass,
			&sip.ToHeader{DisplayName: sip.String{"Alice Liddell"},
				Address: &sip.SipUri{false, sip.String{"alice"}, nil, "wonderland.com", nil, fooNilUP, nil},
				Params:  nil}}},
		...
		{toHeaderInput("To: \"Alice Liddell\" <sip:alice@wonderland.com;foo?foo=bar>;foo=bar"), &toHeaderResult{pass,
			&sip.ToHeader{DisplayName: sip.String{"Alice Liddell"},
				Address: &sip.SipUri{false, sip.String{"alice"}, nil, "wonderland.com", nil, fooNilUP, fooBarUH},
				Params:  fooBarHP}}},
...
func TestContactHeaders(t *testing.T) {
	doTests([]test{
		...
		{contactHeaderInput("Contact: *"), &contactHeaderResult{
			pass,
			[]*sip.ContactHeader{
				{DisplayName: nil, Address: &sip.WildcardUri{}, Params: nil}}}},
		...
		{contactHeaderInput("Contact: sip:alice@wonderland.com, sip:hatter@wonderland.com"), &contactHeaderResult{
			pass,
			[]*sip.ContactHeader{
				{DisplayName: nil, Address: &sip.SipUri{false, sip.String{"alice"}, nil, "wonderland.com", nil, nil, nil}, Params: nil},
				{DisplayName: nil, Address: &sip.SipUri{false, sip.String{"hatter"}, nil, "wonderland.com", nil, nil, nil}, Params: nil}}}},
		...
		{contactHeaderInput("Contact: \"Alice Liddell\" <sips:alice@wonderland.com>, \"Madison Hatter\" <sip:hatter@wonderland.com>"), &contactHeaderResult{
			...
			[]*sip.ContactHeader{
				{DisplayName: sip.String{"Alice Liddell"},
					Address: &sip.SipUri{true, sip.String{"alice"}, nil, "wonderland.com", nil, nil, nil},
					Params:  nil},
				{DisplayName: sip.String{"Madison Hatter"},
					Address: &sip.SipUri{false, sip.String{"hatter"}, nil, "wonderland.com", nil, nil, nil},
					Params:  nil}}}},
		...
		{contactHeaderInput("Contact: \"Alice Liddell\" <sips:alice@wonderland.com>, \"Madison Hatter\" <sip:hatter@wonderland.com>" +
			...
			[]*sip.ContactHeader{
				...
				{DisplayName: sip.String{"Madison Hatter"},
					...
					Params:  nil},
				{DisplayName: nil,
					Address: &sip.SipUri{false, sip.String{"kat"}, nil, "cheshire.gov.uk", nil, nil, nil},
					Params:  nil}}}},
...
func TestViaHeaders(t *testing.T) {
	...
	fooEqBar := sip.NewParams(sip.HeaderParams).Add("foo", sip.String{Str: "bar"})
	fooEqSlashBar := sip.NewParams(sip.HeaderParams).Add("foo", sip.String{Str: "//bar"})
	singleFoo := sip.NewParams(sip.HeaderParams).Add("foo", nil)
	doTests([]test{
		{viaInput("Via: SIP/2.0/UDP pc33.atlanta.com"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"SIP", "2.0", "UDP", "pc33.atlanta.com", nil, nil}}}},
		{viaInput("Via: bAzz/fooo/BAAR pc33.atlanta.com"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"bAzz", "fooo", "BAAR", "pc33.atlanta.com", nil, nil}}}},
		{viaInput("Via: SIP/2.0/UDP pc33.atlanta.com"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"SIP", "2.0", "UDP", "pc33.atlanta.com", nil, nil}}}},
		{viaInput("Via: SIP /\t2.0 / UDP pc33.atlanta.com"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"SIP", "2.0", "UDP", "pc33.atlanta.com", nil, nil}}}},
		{viaInput("Via: SIP /\n 2.0 / UDP pc33.atlanta.com"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"SIP", "2.0", "UDP", "pc33.atlanta.com", nil, nil}}}},
		{viaInput("Via:\tSIP/2.0/UDP pc33.atlanta.com"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"SIP", "2.0", "UDP", "pc33.atlanta.com", nil, nil}}}},
		{viaInput("Via:\n SIP/2.0/UDP pc33.atlanta.com"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"SIP", "2.0", "UDP", "pc33.atlanta.com", nil, nil}}}},
		{viaInput("Via: SIP/2.0/UDP box:5060"), &viaResult{pass, sip.ViaHeader{&sip.ViaHop{"SIP", "2.0", "UDP", "box", &port5060, nil}}}},
...
// Test unstreamed parsing of a sip.Request object (rather than a sip.Response).
...
func (data *paramInput) evaluate() result {
	output, consumed, err := parser.ParseHeaderParams(
		data.paramString,
		data.start,
		data.sep,
		data.end,
...
func (expected *sipUriResult) equals(other result) (equal bool, reason string) {
	...
	if expected.err == nil && actual.err != nil {
		...
		return false, fmt.Sprintf("unexpected success: got \"%+v\"", actual.uri)
	...
	if !equal {
		reason = fmt.Sprintf("expected result %+v, but got %+v", expected.uri, actual.uri)
	}
	return
}
...
func (expected *toHeaderResult) equals(other result) (equal bool, reason string) {
	...
	default:
		...
		// - add a case block above for that schema, using the 'equals' method in the same was as the existing sip.SipUri block above.
		return false, fmt.Sprintf("no support for testing Uri schema in Uri \"%s\" - fix me!", expected.header.Address)
	}

	if !sip.IsParamsEqual(expected.header.Params, actual.header.Params) {
		return false, fmt.Sprintf("unexpected parameters \"%s\" (expected \"%s\")",
			actual.header.Params.String(),
			expected.header.Params.String())
	}
...
func (expected *contactHeaderResult) equals(other result) (equal bool, reason string) {
	...
	for idx := range expected.headers {
		...
		if !sip.IsParamsEqual(expected.headers[idx].Params, actual.headers[idx].Params) {
			return false, fmt.Sprintf("unexpected parameters \"%+v\" (expected \"%+v\")",
				actual.headers[idx],
				expected.headers[idx])
		}
	}
...
func (data cSeqInput) evaluate() result {
	...
	if len(headers) == 1 {
		...
		panic(fmt.Sprintf("Multiple headers returned by sip.CSeq test: %s", string(data)))
	}
}
...
func (data callIdInput) evaluate() result {
	...
	if len(headers) == 1 {
		...
		panic(fmt.Sprintf("Multiple headers returned by sip.CallID test: %s", string(data)))
	}
}
...
func (expected *viaResult) equals(other result) (equal bool, reason string) {
	...
	for idx, expectedHop := range expected.header {
		...
		} else if !sip.IsParamsEqual(expectedHop.Params, actualHop.Params) {
			return false, fmt.Sprintf("unexpected params '%s' in via entry %d - expected '%s'",
				actualHop.Params.String(),
				idx,
				expectedHop.Params.String())
		}
	}
...
func TestParseSipUri(t *testing.T) {
	tests := []struct {
		...
		{"complex one",
			"sips:%61lice;day=tuesday:123abc@atlanta.com:5060;transport=TCP;method=REGISTER?to=sip:bob%40biloxi.com",
			sip.SipUri{
				FIsEncrypted: true,
				FUser:        sip.String{"alice;day=tuesday"},
				FPassword:    sip.String{"123abc"},
				FHost:        "atlanta.com",
				FPort:        lo.ToPtr(sip.Port(5060)),
				FUriParams:   sip.NewParams(sip.UriParams).Add("transport", sip.String{"tcp"}).Add("method", sip.String{"register"}),
				FHeaders:     sip.NewParams(sip.UriHeaders).Add("to", sip.String{"sip:bob@biloxi.com"}),
			}, false},
		{"empty password", "sip:alice:%20@192.168.1.3:7777;transport=udp?to=sip:bob%40biloxi.com",
			sip.SipUri{
				FIsEncrypted: false,
				FUser:        sip.String{Str: "alice"},
				FPassword:    sip.String{Str: " "},
				FHost:        "192.168.1.3",
				FPort:        lo.ToPtr(sip.Port(7777)),
				FUriParams:   sip.NewParams(sip.UriParams).Add("transport", sip.String{"udp"}),
				FHeaders:     sip.NewParams(sip.UriHeaders).Add("to", sip.String{"sip:bob@biloxi.com"}),
			}, false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotUri, err := parser.ParseSipUri(tt.args)
			if (err != nil) != tt.wantErr {
				t.Errorf("ParseSipUri() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotUri, tt.wantUri) {
				t.Errorf("ParseSipUri() gotUri = %+v, want %+v", gotUri, tt.wantUri)
			}
		})
	}
}
