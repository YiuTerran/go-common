...
import (
	...
	"github.com/YiuTerran/go-common/base/util/randutil"
	"strings"
)
...
func (addr *Address) String() string {
	...
	if addr.Params != nil && addr.Params.Length() > 0 {
		buffer.WriteString(";")
		buffer.WriteString(addr.Params.String())
	}
...
func (addr *Address) Equals(other any) bool {
...
func (port *Port) Equals(other any) bool {
	if p, ok := other.(*Port); ok {
		return ptrutil.Equal((*uint16)(port), (*uint16)(p))
	}
	return false
}
...
// IsStringEqual 考虑nil场景的相等判断，空字符串视为nil
func IsStringEqual(lhs, rhs MaybeString) bool {
	if lhs == nil || lhs.String() == "" {
		return rhs == nil || rhs.String() == ""
	}
	if rhs == nil || rhs.String() == "" {
		return lhs == nil || lhs.String() == ""
	}
...
// BrokenMessageError mean broken or incomplete messages, or not a SIP message.
...
// MalformedMessageError syntactically valid but logically invalid message
