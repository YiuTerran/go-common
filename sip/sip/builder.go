...
func NewRequestBuilder() *RequestBuilder {
	callID := CallID(randutil.RandString(32))
...
func (rb *RequestBuilder) SetTransport(transport string) *RequestBuilder {
	if transport == "" {
		...
		rb.transport = strings.ToUpper(transport)
	}
...
func (rb *RequestBuilder) AddVia(via *ViaHop) *RequestBuilder {
	...
	if via.Params == nil {
		via.Params = NewParams(HeaderParams)
	}
...
func (rb *RequestBuilder) Build() (Request, error) {
	...
	hdrs = append(hdrs, rb.from, rb.to, rb.callID, rb.cseq)
