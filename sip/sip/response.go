...
func NewResponse(
	...
	res := new(response)
	if messID == "" {
		res.msgID = NewMessageID()
	} else {
		res.msgID = messID
	}
	res.startLine = res.StartLine
	res.sipVersion = sipVersion
	res.headers = newHeaders(headers)
	res.status = statusCode
	if reason == "" {
		reason = Phrase(statusCode)
	}
	res.reason = reason
	res.body = body
	res.fields = log.MergeFields(fields, log.Fields{
		"response_id": res.msgID,
	})
...
func (res *response) Short() string {
	...
	if cseq, ok := res.CSeq(); ok {
		f["method"] = cseq.MethodName
		f["sequence"] = cseq.SeqNo
	}
	f = f.WithFields(res.Fields()).WithPrefix("sip.Response")
	return f.String()
}
...
func (res *response) WithFields(fields log.Fields) Message {
	res.mu.Lock()
	res.fields = log.MergeFields(res.fields, fields)
	res.mu.Unlock()
...
// NewResponseFromRequest return Request base on RFC 3261 - 8.2.6
...
func cloneResponse(res Response, id MessageID, fields log.Fields) Response {
	newFields := log.MergeFields(res.Fields(), fields)
	newRes := NewResponse(
		id,
		res.SipVersion(),
		res.StatusCode(),
