...
func NewRequest(
	...
	req := new(request)
	if msgID == "" {
		req.msgID = NewMessageID()
	} else {
		req.msgID = msgID
	}
	req.startLine = req.StartLine
	req.sipVersion = sipVersion
	req.headers = newHeaders(headers)
	req.method = method
	req.recipient = recipient
	req.body = body

	req.fields = log.MergeFields(fields, log.Fields{
		"request_id": req.msgID,
	})
	return req
}
...
func (req *request) Short() string {
	...
	fields = log.MergeFields(req.Fields(), fields)

	return fields.WithPrefix("sip.Request").String()
}
...
func (req *request) WithFields(fields log.Fields) Message {
	req.mu.Lock()
	req.fields = log.MergeFields(req.fields, fields)
	req.mu.Unlock()
...
func NewAckRequest(ackID MessageID, inviteRequest Request,
	inviteResponse Response, body string, fields log.Fields) Request {
	...
	ackRequest := NewRequest(
		...
		[]Header{},
		body,
		log.MergeFields(inviteRequest.Fields(), fields, log.Fields{
...
func NewCancelRequest(cancelID MessageID, requestForCancel Request, fields log.Fields) Request {
	cancelReq := NewRequest(
		...
		[]Header{},
		"",
		log.MergeFields(requestForCancel.Fields(),
			fields, log.Fields{
...
func cloneRequest(req Request, id MessageID, fields log.Fields) Request {
	...
	if fields != nil {
		newFields = log.MergeFields(newFields, fields)
	}
