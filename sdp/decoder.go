...
func (d *Decoder) media(m *Media, f byte, v string) error {
	...
	case 'f':
		mf := &MediaFormat{}
		parts := strings.Split(v, "a")
		if len(parts) != 2 {
			return fmt.Errorf("sdp f format error:%s", v)
		}
		vs := strings.Split(parts[0], "/")
		as := strings.Split(parts[1], "/")
		if len(vs) != 6 || len(as) != 4 || vs[0] != "v" || as[0] != "" {
			return fmt.Errorf("sdp f format error:%s", v)
		}
		mf.VideoEncoder, _ = strconv.Atoi(vs[1])
		mf.ResolutionType, _ = strconv.Atoi(vs[2])
		mf.VideoFrameRate, _ = strconv.Atoi(vs[3])
		mf.VideoBitRateType, _ = strconv.Atoi(vs[4])
		mf.VideoBitRate, _ = strconv.Atoi(vs[5])
		mf.AudioEncoder, _ = strconv.Atoi(as[1])
		mf.AudioBitRate, _ = strconv.Atoi(as[2])
		mf.AudioSampleRate, _ = strconv.Atoi(as[3])
		m.MediaFormat = mf
	default:
		return errUnexpectedField
	}
...
func (d *Decoder) timezone(v string) ([]*TimeZone, error) {
	...
	for len(p) > 1 {
		it := new(TimeZone)
		if it.Time, err = d.int(p[0]); err != nil {
			return nil, err
		}
