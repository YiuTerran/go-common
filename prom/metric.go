...
func (p *Prometheus) sendMetricsToPushGateway(metrics []byte) {
	...
	if _, err = client.Do(req); err != nil {
		log.Error("Error sending to push gateway:%s", err)
	}
}
...
func (p *Prometheus) registerMetrics(subsystem string) {
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			log.Error("%s could not be registered in Prometheus:%s", metricDef.Name, err)
		}
		switch metricDef {
		case reqCnt:
...
func (p *Prometheus) Use(e *gin.Engine, path ...string) {
	e.Use(p.HandlerFunc())
	if len(path) > 0 {
		p.MetricsPath = path[0]
	}
